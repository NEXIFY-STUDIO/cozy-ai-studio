/**
 * React binding for P2PRoom — identity/room captured once on mount.
 */
...
export function useP2PRoom(options: UseP2PRoomOptions = {}): P2PRoomHandle {
  ...
  const listeners = useRef(
    new Set<
      (from: string, data: unknown, channel: "state" | "reliable") => void
    >(),
  );
  ...
  const broadcast = useCallback(
    (data: unknown) => roomRef.current?.broadcast(data),
    [],
  );
