...
//#region node_modules/.nitro/vite/services/ssr/assets/studio-store-COsw9EQn.js
var STARTER_APP = `import React from "react";
...
`;
var STARTER_STYLES = `/* Warm Brutalism tokens */
:root {
  --cream: #F4F1EA;
  --charcoal: #1C1D21;
  --terracotta: #D96B43;
...
function buildPreviewHtml(brand, headline, sub) {
	return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
var STARTER_PREVIEW = buildPreviewHtml("Aurora", "Design that feels handmade", "A starter landing page generated inside COSY Studio.");
var initialFiles = {
	"src/App.tsx": {
		path: "src/App.tsx",
		language: "typescript",
...
function computeChunks(original, modified) {
	...
	for (let line = 0; line < max; line++) if ((oLines[line] ?? "") !== (mLines[line] ?? "")) {
...
var useStudioStore = create()(persist((set, get) => ({
	...
	chat: [{
		id: "welcome",
		role: "system",
		content: "COSY multi-agent pipeline ready. Describe a UI change — G0 plans, G1 codes, G2 audits.",
		timestamp: Date.now()
	}],
	...
	setPlanTier: (tier) => set({
		planTier: tier,
		promptLimit: tier === "FREE" ? 100 : 9999
	}),
	setAgents: (agents) => set({ agents }),
	updateAgent: (id, patch) => set({ agents: get().agents.map((a) => a.id === id ? {
		...a,
		...patch
	} : a) }),
	addChat: (msg) => set({ chat: [...get().chat, {
		...msg,
		id: `msg-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
		timestamp: Date.now()
	}] }),
	setDiff: (original, modified, language) => set({
		originalCode: original,
		modifiedCode: modified,
		language: language ?? get().language,
		diffChunks: computeChunks(original, modified)
	}),
	streamModifiedCode: (code) => set({
		modifiedCode: code,
		diffChunks: computeChunks(get().originalCode, code)
	}),
	acceptChunk: (chunkId) => set({ diffChunks: get().diffChunks.map((c) => c.id === chunkId ? {
		...c,
		accepted: true
	} : c) }),
	rejectChunk: (chunkId) => set({ diffChunks: get().diffChunks.map((c) => c.id === chunkId ? {
		...c,
		accepted: false
	} : c) }),
	acceptAllDiffs: () => {
	...
	setPipelineProgress: (pct, label) => set({
	...
	cancelPipeline: () => {
		...
		get().addChat({
			role: "system",
			content: "Pipeline cancelled by user."
		});
	},
	incrementPrompts: () => set({ promptsUsed: get().promptsUsed + 1 }),
	setPublishUrl: (url) => set({ publishUrl: url }),
	addTelemetry: (log) => set({ telemetry: [{
		...log,
		id: `tel-${Date.now()}`,
		createdAt: Date.now()
	}, ...get().telemetry].slice(0, 50) }),
	updateFileContent: (path, content) => {
		const files = { ...get().files };
		files[path] = {
			path,
			language: files[path]?.language ?? "typescript",
			content
		};
		set({ files });
	},
...
//#endregion
export { useStudioStore as n, buildPreviewHtml as t };
